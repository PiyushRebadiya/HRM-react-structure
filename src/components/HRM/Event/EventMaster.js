...
function EventNew(props) {
  const { fetchData, getEventData } = props;
  return (
    <Modal
      {...props}
      size="xl"
      ...
      backdrop="static"
    >
      <EventForm onHide={props.onHide} fetchData={fetchData} getEventData={getEventData} />
    </Modal>
  );
}

const EventMaster = ({ getEventData, onHide }) => {
  const [eventnew, setEventNew] = React.useState(false);
  ...
  return (
    <div className={getEventData ? "" : 'content-wrapper h-75'} >
        ...
        <div className="row">
          <div className="col-lg-12 pinpin">
            <div className="card lobicard" data-sortable="true">
              <div className="card-header">
                <div className='title-download-section'>
                  <div className="card-title custom_title">
                    <h4 className='report-heading'>Event List</h4>
                  </div>
              ...
              <div className="btn-group d-flex input-searching-main pt-3 pl-3 ps-3" role="group">
                <div className="buttonexport" id="buttonlist">
                  <Button className="btn btn-add rounded-2"
                    onClick={() => setEventNew(true)}
                  >
                    <i className="fa fa-plus" /> Add Event [F2]
                  </Button>
                  <EventNew
                    show={eventnew}
                    onHide={() => setEventNew(false)}
              ...
              <div className='p-3' >
                <EventTable insertData={insertData} searchinput={searchinput} onData={handleData} getEventData={getEventData} />
              </div>
            </div>
          </div>
        </div>
